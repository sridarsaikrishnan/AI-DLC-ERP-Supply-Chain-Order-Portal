...
- **FR-37** Onboarding a reseller creates the tenant, its OAuth client in Keycloak through the Admin API with the tenant identifier as a token claim, its bindings and any item ownership. (P)
...
- **NFR-12** Infrastructure resilience is deliberately MVP-level (backup and restore, single region). Application-level reliability is not reduced: a durable queue, retry with backoff, idempotent processing and a transactional outbox remain required (NFR-01).

### 4.2 Technology and deployment (see Section 5)
- **NFR-05** Portable-core rule: application code depends only on PostgreSQL, Kafka, OIDC, HTTP and OpenTelemetry. It calls no AWS-specific API. Any new AWS-only dependency needs a local story first. (P)
- **NFR-06** Configuration is delivered as environment variables: injected from AWS Secrets Manager by ECS in AWS, from a git-ignored `.env` file locally. ERP credentials are AES-GCM encrypted in PostgreSQL with a key from an environment variable and a stored key identifier for rotation. (D, P)
...
|---|---|
| SECURITY-01 | Encrypt at rest and in transit (TLS 1.2+) for RDS PostgreSQL, MSK Kafka, backups, Terraform state and any bucket. Managed keys. |
...
| SECURITY-04 | Security headers on all HTML endpoints (CSP, HSTS, nosniff, frame options, referrer policy), including the UI container and Keycloak. The UI self-hosts scripts and fonts. |
...
| SECURITY-07 | Deny-by-default networking. ECS tasks, RDS and MSK in private subnets. The only public inbound is the ALB on 80/443. Keycloak admin console not publicly reachable. |
...
| SECURITY-09 | No default credentials (including the Keycloak bootstrap admin). Generic production errors. Current supported runtime versions. No sample apps deployed. |
...
| SECURITY-12 | Keycloak enforces password policy with breached-password checks and adaptive hashing, MFA for admin accounts, session expiry and logout, brute-force protection. No hardcoded secrets; Secrets Manager is the store, injected as environment variables. |
...
|---|---|---|
| RESILIENCY-01 | Classify `api`, `worker`, Keycloak, PostgreSQL and Kafka as Critical and the UI as High; document business impact and dependencies. | Proposed, confirm |
...
| RESILIENCY-04 | A CI/CD pipeline proposed for Terraform and .NET containers; rollback by redeploying the previous version; rolling deployment. | Decided (Q25 = B, Q26 = A, Q27 = B) |
| RESILIENCY-05 | Metrics, structured logs, distributed traces and a health dashboard. | Requirement captured |
| RESILIENCY-06 | Shallow and deep health checks (database, Kafka, Keycloak) wired to ALB health checks. | Requirement captured |
...
| RESILIENCY-08 | Single region, multi-zone: `api` and `worker` tasks in 2 or more zones, RDS Multi-AZ, MSK across zones. Keycloak zone redundancy to be resolved in NFR Design (O-10). | Decided (Q23 = A) |
...
| RESILIENCY-11..13 | Backup and Restore: automated encrypted RDS backups with point-in-time recovery, retention policy, periodic test restore, and a runbook to redeploy from Terraform and restore. Outbox retention to allow republishing to Kafka (validate in Application Design). | Requirement captured |
...
- **NFR-10** Framework: FsCheck with xUnit (P, confirm at NFR Requirements). Shrinking enabled, seed logged on every run, PBT runs in CI. Reusable domain generators. Example-based tests also pin every business-critical path.
...
|---|---|---|
| Backend | .NET (ASP.NET Core, GraphQL with HotChocolate) | Q12 = D, D |
| Frontend | React | Q13 = A |
| Cloud | AWS | Q14 = A, D |
| Compute | ECS Fargate containers: `api`, `worker`, `ui`, Keycloak (modular monolith, two deployables plus UI) | P |
| Datastore | PostgreSQL (RDS) with row-level tenant isolation | Q15 = C, P |
| Messaging | Apache Kafka (Amazon MSK), partitioned by order ID, transactional outbox, retry topics and dead-letter topic | D, P |
| Identity | Keycloak (container locally and on ECS), realm as code | D |
...
| Configuration | Environment variables; ERP credentials encrypted in PostgreSQL | D, P |
| Observability | OpenTelemetry to CloudWatch/X-Ray in AWS, Jaeger or Grafana locally | P |
| Local environment | Docker Compose: PostgreSQL, Kafka (KRaft), Keycloak, mock ERP, optional Odoo and ERPNext profile | D, P |
| Testing | xUnit, FsCheck, Testcontainers; tiers: unit, integration, ERP conformance, post-deploy smoke test in a dev AWS account | P |
...
| O-10 | RESILIENCY-08 needs compute in 2 or more zones, but Keycloak was planned as a single instance for the MVP. Run 2 tasks with clustering, or document an exception. | NFR Design |
...
- **Resiliency Baseline**: enabled. No blocking findings at this stage: the decisions required by RESILIENCY-02, 03, 04, 08 and 15 are recorded at MVP level (Q22-Q28) and marked for confirmation. RESILIENCY-14 is asked at NFR Design. Open risk: O-10.
